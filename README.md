...
1. **数値検出**: EasyOCR で画像内のすべての数値を検出
2. **フィルタリング**: 1〜999 の範囲の数値のみを棚番号として認識
3. **最大値抽出**: 検出された数値の最大値を棚数として採用
4. **信頼度**: OCR 信頼度 0.5 以上の検出結果のみを使用

## 📊 解析精度について

- OCR による自動カウントのため、実際の棚数と誤差が生じる可能性があります
- フロアマップの画像品質や棚番号の表記方法により精度が変動します
- より正確な調査には手動確認を推奨します
...
現在は棚数カウントによる概算ですが、快活空間のコミック検索機能 API を活用することで、以下のような詳細調査が可能です：

1. **特定タイトルの在庫確認**: 人気漫画リストを作成し、各店舗での在庫有無を調査
2. **ジャンル別蔵書数**: 少年漫画、少女漫画、青年漫画などカテゴリ別の充実度を比較
3. **新刊対応速度**: 新刊の入荷タイミングを店舗間で比較
4. **レア本調査**: 絶版・希少本の所蔵店舗をマッピング

### 技術的改善案

- 画像認識の精度向上（ディープラーニングモデルの適用）
- 並列処理による高速化
- WebUI ダッシュボードの開発
- 定期自動実行とデータ蓄積
